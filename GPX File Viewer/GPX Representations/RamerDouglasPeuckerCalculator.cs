namespace GPX_File_Viewer.GPX_Representations
{
    ...
    public class DouglasPeuckerInterpolation
    {
        ...
        private static double GetDistance(WayPoint start, WayPoint end, WayPoint point)
        {
            var x = end.X - start.X;
            var y = end.Y - start.Y;
        ...
        ///     List{WayPoint}, int, double)"/> part of the algorithm.
        /// </remarks>
        private static Segment CreateSegment(int start, int end, List<WayPoint> points)
        {
            var count = end - start;
        ...
        private static bool IsValid(WayPoint point)
        {
        ...
        public static IEnumerable<WayPoint> Interpolate(List<WayPoint> points, int max,
            double tolerance = 0d)
        {
        ...
        private static IEnumerable<WayPoint> GetPoints(Segment segment, int count,
            int index, List<WayPoint> points)
        {
        ...
        private static IEnumerable<Segment> GetSegments(List<WayPoint> points)
        {
            var previous = 0;
        ...
        private static void Reduce(ref List<Segment> segments, List<WayPoint> points,
            int max,
            double tolerance)
        {
